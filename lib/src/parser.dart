...
SubRegionType getSubRegionType(SubRegion subRegion, Chunk currentToken) {
  ...
  if (currentToken.type == TOKEN_TYPE.DECIMAL) return SubRegionType(type: TOKEN_TYPE.DECIMAL, isHundred: isHundred);
  if (subRegion.type == TOKEN_TYPE.MAGNITUDE) return SubRegionType(type: TOKEN_TYPE.MAGNITUDE, isHundred: isHundred);
...
SubRegionFit checkIfTokenFitsSubRegion(SubRegion subRegion, Chunk token, Options options) {
  ...
  if (subRegion == null) return SubRegionFit(action: TokenFit.START_NEW_REGION, type: type, isHundred: isHundred);
...
class SubRegionFit {
  ...
  SubRegionFit({this.action, this.type, this.isHundred = false});
}

List<SubRegion> getSubRegions(Region region, Options options) {
  final subRegions = List<SubRegion>();
  ...
  while (i >= 0) {
    ...
    final action = subRegionFit.action, type = subRegionFit.type, isHundred = subRegionFit.isHundred;
...
List<Region> parse(String text, Options options) {
  final chunks = splitKeepDelimeters(text, new RegExp('(\\w+|\\s|["$puncExcludingDoubleQuote])', caseSensitive: false));
  final tokens = List<Chunk>();
  for (String chunkText in chunks) {
    final unfuzzyChunk = chunkText.isNotEmpty && options.fuzzy && !PUNCTUATION.contains(chunkText) ?
        fuzzyMatch(chunkText) :
        chunkText;
...
class Chunk {
  ...
  Chunk({@required this.start, @required this.end, @required this.value, @required this.lowerCaseValue, @required this.type,});
}
...
class Region {
  ...
  Region({@required this.start, @required this.end, @required this.tokens,});
